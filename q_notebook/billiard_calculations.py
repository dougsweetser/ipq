...
# This is a case where the non-reduced form is more convenient. The time squared is about 60 quadrillion while the change in space squared is slighly over 64. Classical physics is full of such imbalances and the non-reduced form helps maintian the separation.
...
# For the gray set, the events from Observer B are being compared with a boosted Observer B for motion that is only along the $x$ direction. We thus expec the $y$ and $z$ values to be exact as they are (down exact and here exact because $z=0$). The value of $x$ is boosted, so they are both right, but not the same value. But what about time? The report is for an exact match. The software was written to say two values were equivalent if they were the same to 10 significant digits. It is the 16th significant digit which is different.

# The time-like interval is the same for Observer B and the boosted one, so the equivalence class is timelike-exact as expected. This graphics are icons to represent the class, not a reflection of the numbers used. The space-times-time terms are only different along $t x$ due to the boost along $x$.

# The norm is included for completeness of simple operartions, but I don't understand it at this time. It is marked as exact due to the dominance of the time term.
...
# More precise measurements of weak field gravity adds a few more terms (essentiall eq. 40.1 of Misner, Thorne and Wheeler):
...
# The sixth term is not the same for the Taylor series expansion of the Schwarzschild solution in either isotrophic or Schwarzschild coordinates. It is not resonable to expect the simple space-times-time equivalence constraint will solve the non-linear Einstein field equations.

# The truncated series expansion will not be the final story. We could wait for experimentalist to determine 10 terms, but that is quite unresonable (Personal story: I spend ~$600 to go to an Eastern Gravity Meeting just to ask Prof. Clifford Will when we might get the terms for second order Parameterize Post-Newtonian accuracy, and at the time he knew of no such planned experimental effort ~2005?). Given that gravity is o harmonic phenomena, there are six terms that match, and many other people have made the same speculation, it is a small leap to suggest that a postive and negative exponential to the dimensionless mass length may be the complete solution for simple systems:
...
# The squares could be calculated, but if the imput values are the same, there will be no difference in any of the squares. This is consistent with expectations: a small number change in a small number cannot be noticed.
...
# Nearly all calculations in physics I have ever done involved vector spaces over a mathematical field like real or complex numbers. This iPython notebook used a fundamentally new and different toolset. Space-time numbers can always be made equivalent to the 4D division algebra of quaternions through the process of reduction. This new breed of numbers was applied to the simple case of an eightball moving along a pool table. Threee observers watched the events unfold. Their raw data always was different because that is inherent in the definition of being a different observer. Yet by taking the difference between two events and squaring it, equivalence classes could be defined. If two observers agree on the first term of the square, that means the two observers are moving at a constant speed relative to each other. The other three terms can be used to figure out the motion. While this might be view as a novel approach to special relativity, nothing new is claimed.

# If two observers are in an equivlance class because the difference between two events squared space-times-time values are exactly the same, this is a new proposal for how gravity works. There is no metric tensor, connection, nor field equations. Rather, the proposal is a twin of special relativity. In both cases it is the square that matters. The difference between the two is if the observers agree to the reduced real values, that is special relativity, where if they agree to the reducded imaginary values, that is space-times-time equivalence class as gravity.
